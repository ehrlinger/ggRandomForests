...
ggpart <- pbc_ggpart[["edema"]]
plot(ggpart, notch=TRUE, alpha=.3, outlier.shape = NA) + 
...
copper_cts <- quantile_cuts(ggvar$copper, groups=25)
copper_grp <- cut(ggvar$copper, breaks=copper_cts)
@
...
system.time(partial_coplot_pbc_surf <- 
            gg_partial_coplot(rfsrc_pbc, xvar="bili", time=1,
                              groups=copper_grp, npts=25,
                              show.plots=FALSE))

# user  system elapsed 
# 494.971  17.153 516.469 
...
copper.tmp <- do.call(c,lapply(copper_cts[-1], 
                               function(grp){rep(grp, 25)}))

# attach the data to the gg_partial_coplot
partial_coplot_pbc_surf$copper <- copper.tmp

# ggplot2 contour plot of x, y and z data.
ggplot(partial_coplot_pbc_surf, aes(x=bili, y=copper, z=yhat))+
  stat_contour(aes(colour = ..level..), binwidth = 2.5)+
...
srf <- surface_matrix(partial_coplot_pbc_surf, c("bili", "copper", "yhat"))
...
<<def-pts2>>=
# Find the quantile points to create 30 interval groups
bili_cts <- quantile_cuts(rfsrc_pbc$xvar$bili, groups=25)
bili_grp <- cut(rfsrc_pbc$xvar$bili, breaks=bili_cts)
@
...
# Generate the gg_partial_coplot data object
system.time(partial_coplot_pbc_surf2 <- 
            gg_partial_coplot(rfsrc_pbc, xvar="copper", time=1,
                              groups=bili_grp, npts=25,
                              show.plots=FALSE))

#    user  system elapsed 
# 492.414  16.024 508.824 
...
# Load the stored partial coplot data.
data(partial_coplot_pbc_surf2)

# Instead of groups, we want the raw rm point values,
# To make the dimensions match, we need to repeat the values
# for each of the 50 points in the lstat direction
bili.tmp <- do.call(c,lapply(bili_cts[-1], 
                               function(grp){rep(grp, 25)}))

# attach the data to the gg_partial_coplot
partial_coplot_pbc_surf2$bili <- bili.tmp

# ggplot2 contour plot of x, y and z data.
ggplot(partial_coplot_pbc_surf2, aes(x=bili, y=copper, z=yhat))+
  stat_contour(aes(colour = ..level..), binwidth = 2.5)+
  labs(x=st.labs["bili"], y=st.labs["copper"], 
       color="Survival at 1 year (%)")+
  scale_colour_gradientn(colours=topo.colors(10))
...
# Transform the gg_partial_coplot object into a list of three named matrices
# for surface plotting with plot3D::surf3D
srf <- surface_matrix(partial_coplot_pbc_surf2, c("bili", "copper", "yhat"))
