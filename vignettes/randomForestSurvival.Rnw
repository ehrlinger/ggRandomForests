...
ind=1
ggrf <- gg_variable(pbc_rf, time=c(1,3)*364.25, time.labels = c("1 Year", "3 Years"))

plot(ggrf, x_var = xvar[ind]) +
...
plot(ggrf, x_var = xvar[2:5], panel=TRUE) +
  labs(y="Survival")+
  theme(legend.position="none")+
  scale_color_manual(values=strCol, na.value = "black", drop=FALSE,
                     labels=event.labels)+
...
## Need to combine multiple time curves along variables.
pbc_ggPart <- combine(ggPrtl, ggPrtl.3, labels=c("1 Year", "3 Years"))
...
nm  <- "bili"
plot(pbc_ggpart[[nm]], se=FALSE)+
  theme(legend.position=c(.8,.5))+
  labs(y="Survival", 
       x=dta.labs[which(rownames(dta.labs)==nm), "label"])
@

<<pbc-partial-panel, echo=TRUE, fig.cap="Risk adjusted Survival", fig.width=7, fig.height=4>>=
...
plot(gg_interaction(pbc_interaction), x_var=xvar[1])+
