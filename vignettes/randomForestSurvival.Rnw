...
gg_src <- plot.gg_rfsrc(pbc_rf,   level=.95) + 
  scale_color_manual(values = strCol) + 
  theme(legend.position = "none") + 
  labs(y = "Survival Probability", x = "time (years)")+
  coord_cartesian(ylim=c(-.01,1.01))
...
plot(ggrf, xvar = xvar[c(2,3,5,6)], panel = TRUE, 
     se=FALSE, alpha=.3, 
...
plot(gg_interaction(pbc_interaction), xvar = xvar[2:5]) + 
  labs(y = "Interactive Minimal Depth") + 
  theme(legend.position = "none")
@
...
<<build-bili-age, eval=FALSE>>=
pbc_prtl_bili_age <- gg_partial_coplot(pbc_rf, xvar="bili", 
                                       groups=age_grp, 
                                       surv_type="surv", 
                                       time=1, 
                                       show.plots=FALSE)
...
pbc_prtl_bili_albumin <- gg_partial_coplot(pbc_rf, xvar="bili", 
                                       groups=albumin_grp, 
                                       surv_type="surv", 
                                       time=1, 
                                       show.plots=FALSE)
...
<<build-bili-prothrombin, eval=FALSE>>=
pbc_prtl_bili_prothrombin <- gg_partial_coplot(pbc_rf, xvar="bili", 
                                       groups=prothrombin_grp, 
                                       surv_type="surv", 
                                       time=1, 
                                       show.plots=FALSE)
@
