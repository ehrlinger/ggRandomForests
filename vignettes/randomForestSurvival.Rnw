...
plot(ggrf, xvar = xvar[-1], panel = TRUE, 
     se=FALSE, alpha=.3, 
...
partial_coplots_pbc <- gg_partial_coplot(rfsrc_pbc, xvar="bili", 
                                         groups=albumin_grp, 
                                         surv_type="surv", 
                                         time=1, 
                                         show.plots=FALSE)
...
data(partial_coplots_pbc, package="ggRandomForests")
ggpl <- ggplot(partial_coplots_pbc, aes(x=bili, y=yhat, 
                                          shape=group, 
                                          color=group))+
