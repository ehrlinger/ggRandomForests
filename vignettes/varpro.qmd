...
before reporting (the importance z is pre-filtered), so the final ranking
...
**`gg_varpro()`** summarizes the per-tree importance distribution.
...
(it only supports `regr` and `class`). `gg_ivarpro()` also supports only
regression and classification fits. Neither wrapper computes local or
lasso-beta importance for a survival outcome, where the response records an
event time and whether the event was observed.

If you call either wrapper on a survival fit, the error names the unsupported
family. The family-support matrix in the closing reference section records
this. For survival fits, use the parts of the toolkit that do work:
`gg_varpro()`, `gg_partial_varpro()`, and `gg_isopro()`.
...
A `varpro` fit narrows your predictors twice, and telling the two narrowings
apart matters the moment you want a partial-dependence curve for a variable
that didn't make the ranking.
...
| `object$xvar.names` | the variables `varPro::partialpro()` can reach |
...
All three coincide. This fit uses the completeness settings recommended below,
so every predictor is reachable and the ranking runs to its tail instead of
stopping at the screened top. That is what fitting with both screens off buys
you, and it is why the counts here are boring. On varPro's defaults they come
apart, which the rest of this section is about.

Even where the ranking is the shorter list, a curve for a variable outside it
is a matter of naming it:

```{r topvars-widen, eval=FALSE}
...
The second narrowing is the one that surprises people. Guided tree-splitting
(`split.weight = TRUE`, the default) runs a preliminary lasso to decide which
variables are worth splitting on, and a variable that never gets split on never
lands in `object$xvar.names`. It is gone well before `partialpro()` sees it.
The binary iris fit, left on varPro's defaults so it has something to show,
is this in miniature:

```{r varpro-counts-iris}
c(handed_in = ncol(v_iris_binary$x),
  reachable = length(v_iris_binary$xvar.names))
```

Four predictors go in and one of them never becomes a candidate. With p in the
hundreds the gap is not small.

One sharp edge. `partialpro()` matches your `xvar.names` against
`object$xvar.names` and drops whatever it cannot find. Ask for twelve variables
and you may get ten back. From varPro 3.3.0 it warns and names them, but only
after the computation has run. A misspelled name behaves the same way. To find
out before you spend the time, check:

```{r partialpro-check, eval=FALSE}
