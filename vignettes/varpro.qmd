...
| `object$xvar.names` | the variables `partialpro()` can reach |
| `get.topvars(object)` | the reported ranking, and the default for `xvar.names` |
...
Every predictor stays reachable here; the importance pre-filter only trims what
gets *reported*. A curve for a variable outside the reported ranking is a
matter of naming it:
...
The binary iris fit shows this in miniature:
