...
```{r libs}
# ggRandomForests is auto-attached by the vignette engine; explicit
# library(ggRandomForests) breaks the Windows R CMD check vignette
# rebuild because the check-time install library isn't on .libPaths()
# for that subprocess. The other vignettes follow the same pattern.
# All varPro calls below are :: qualified, so no library(varPro) either.
library(ggplot2)
```
...
@Ishwaran:2007a. Each wrapper's help page carries a "What this is
