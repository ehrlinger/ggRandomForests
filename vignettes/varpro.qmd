...
`object$xvar.names` and quietly drops whatever it cannot find. Ask for twelve
variables and you may get ten back, with no error, no warning, and nothing on
the returned object to say so. A misspelled name behaves the same way. When it
matters, check:
