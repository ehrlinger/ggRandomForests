...
\begin{Soutput}
       ...
       Average no. of terminal nodes: 78.353
...
R> 
R> plot(ggrf, x_var = xvar[ind], se=.95, alpha=.3) + 
+   labs(y = "Survival") + 
...
\begin{Schunk}
\begin{Sinput}
R> plot(ggrf, x_var = "edema", notch=TRUE) + 
+   labs(y = "Survival") + 
+   theme(legend.position = "none") + 
+   scale_color_manual(values = strCol, labels = event.labels) + 
+   scale_shape_manual(values = event.marks, labels = event.labels)+
...
R> ggvar$treatment[which(ggvar$treatment==1)] <- "DPCA" 
...
R> pbc_prtl_bili_age$age <- paste("Age=", gg_merge$age)
R> pbc_prtl_bili_age$age <- factor(gg_merge$age)
R> 
