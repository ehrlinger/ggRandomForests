...
R> 
R> plot(ggrf, x_var = xvar[1], se=.95, alpha=.3) + 
+   labs(y = "Survival") + 
...
\appendix
\section{randomForestSRC code snippets}

If we copy and paste a section of code often enough, eventually it should dawn on is a function would be preferable. Until I build this particualr function, these three code snippets were used to build the partial coplots for bilirubin-age (Figure~\ref{fig:bili-age}), bilirubin-albumin (Figure~\ref{fig:bili-albumin}) and bilirubin-prothrombin (Figure~\ref{fig:bili-prothrombin}). 

\subsection{Building a conditional coplot data object: Bilirubin-Age dependency.}
\begin{Schunk}
\begin{Sinput}
...
+ }
R> pbc_prtl_bili_age <- do.call(rbind, gg_part)
R> pbc_prtl_bili_age$age <- paste("Age=", pbc_prtl_bili_age$age)
R> pbc_prtl_bili_age$age <- factor(pbc_prtl_bili_age$age)
\end{Sinput}
\end{Schunk}


\subsection{Building a conditional coplot data object: Bilirubin-Albumin dependency.}
\begin{Schunk}
\begin{Sinput}
R> # Get the training data to work with...
R> dta.train  <- pbc_rf$xvar
R> dta.train$albumin_grp <- ggvar$albumin_grp
R> 
R> # Create a series of coplot subsets....
...
+ }
R> pbc_prtl_bili_albumin <- do.call(rbind, gg_part)
R> pbc_prtl_bili_albumin$albumin <- paste("albumin=", pbc_prtl_bili_albumin$albumin)
R> pbc_prtl_bili_albumin$albumin <- factor(pbc_prtl_bili_albumin$albumin)
\end{Sinput}
\end{Schunk}

\subsection{Building a conditional coplot data object: Bilirubin-Prothrombin dependency.}

\begin{Schunk}
\begin{Sinput}
R> # Get the training data to work with...
R> dta.train  <- pbc_rf$xvar
R> dta.train$prothrombin_grp <- ggvar$prothrombin_grp
R> 
R> # Create a series of coplot subsets....
...
R> gg_merge$prothrombin <- paste("prothrombin=", pbc_prtl_bili_prothrombin$prothrombin)
R> pbc_prtl_bili_prothrombin$prothrombin <- factor(pbc_prtl_bili_prothrombin$prothrombin)
\end{Sinput}
\end{Schunk}
