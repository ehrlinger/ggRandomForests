...
It is good practice to view your data before beginning an analysis, what [@Tukey:1977] refers to as Exploratory Data Analysis (EDA). To facilitate this, we use the ggplot2::facet_wrap commands to create two sets of panel plots, one for categorical variables, and one for continuous variables. Each variable is plotted along one selected continuous variable on the X-axis. These figures help to find outliers and missing values by exploring each variable panel before getting to deep into the analyis. We have created a [Shiny](shiny.rstudio.com) app to make this process easier, available at (https://ehrlinger.shinyapps.io/xportEDA) to create similar figures for arbitrary data set. 

In the Boston housing case, the data consists almost entirely of continuous variables, with the exception of tracts along the Charles river. A simple visualization for this data is then a single panel plot of continuous variables, with observations colored by the logical Charles river variable.  Missing values in each variable are indicated by the rug marks along the x-axis. We use the Boston housing response variable, medv - the median value of homes for X variable.
...
plot(gg_rfsrc(Boston_rfsrc), alpha=.5)+
...
plot(gg_interaction(Boston_int), xvar=Boston_var$topvars[1:6], panel=TRUE)+
