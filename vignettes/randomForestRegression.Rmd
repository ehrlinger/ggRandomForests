...
yhat.tmp <- lapply(unique(partial_coplot_Boston_surf$group), 
                   function(grp){partial_coplot_Boston_surf$yhat[which(partial_coplot_Boston_surf$group==grp)]})
rm.tmp <- lapply(unique(partial_coplot_Boston_surf$group), 
                 function(grp){partial_coplot_Boston_surf$rm[which(partial_coplot_Boston_surf$group==grp)]})
lstat.tmp <- lapply(unique(partial_coplot_Boston_surf$group), 
                    function(grp){partial_coplot_Boston_surf$lstat[which(partial_coplot_Boston_surf$group==grp)]})

rm.tmp <- do.call(rbind, rm.tmp)
lstat.tmp <- do.call(rbind, lstat.tmp)
yhat.tmp <- do.call(rbind, yhat.tmp)

surf3D(x=lstat.tmp, y=rm.tmp, z=yhat.tmp,
       colkey=FALSE, border = "black",
       bty="b2", xlab="Lower Status", ylab="Average Rooms", zlab="Median Value")
```
