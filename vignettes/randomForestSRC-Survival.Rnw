...
kable(tmp, 
      format="latex",
      caption = "\\label{T:dataLabs}\\code{pbc} variable dictionary.",
      row.names = FALSE,
      booktabs=TRUE)
@
...
<<categoricalEDA, fig.cap="Categorical variable EDA plots (logicals and factors). Bars indicate counts within 1 year of followup for each categorical variable. Bars are colored according to the class membership within each variable. Missing values are included as white bars.", fig.width=7>>= 
## Not displayed ##
...
<<predictPlot, fig.cap="Test set prediction: 106 observations with missing value imputation. Blue lines correspond to censored observations, red lines correspond to patients who experienced the event (death).", echo=TRUE>>= 
...
<<pbc-partial-panel, echo=TRUE, fig.cap="Partial dependence plot of (risk adjusted) predicted survival probability as a function continuous variables at 1 year (red circle) and 3 years (blue triangle).", fig.width=5, fig.height=5>>= 
...
<<pbc-partial-edema, echo=TRUE, fig.cap="Partial dependence plot of (risk adjusted) predicted survival probability as a function of \\code{edema} (categorical variable) at 1 year (red) and 3 years (blue). Boxplots indicate distribution of risk adjusted prediction for all patients within each edema group.">>= 
...
\subsection{Partial Dependence in the Time dimension}\label{S:timeSurface}

In the previous section, we calculated and showed risk adjusted (partial) dependence at two time points (1 and 3 years). The selection of these points can be driven by biological times of interest (i.e.\ 1 year and 5 year survival in cancer studies) or by investigating time points of interest from a \code{gg_rfsrc} prediction plot. We typically restrict generating \code{gg_partial} plots to the variables of interest and two or three time points of interest due to computational constraints. 
...
<<albumin-bili, fig.cap="Partial (risk adjusted) variable dependence coplot. Survival at 1 year against \\code{albumin}, stratified by \\code{bili} groups. Points mark risk adjusted estimates, loess smooth indicates predicted trend within each group as a function of \\code{albumin}.", fig.width=7, fig.height=4, echo=FALSE>>= 
# Load cached partial plot data
...
surf3D(x = srf$x, y = srf$y, z = srf$z, col = topo.colors(25),
       colkey = FALSE, border = "black", bty = "b2", 
       shade = 0.5, expand = 0.5, theta=65, phi=15,
       lighting = TRUE, lphi = -50,
       xlab = "Bilirubin", ylab = "Albumin", zlab = "Survival at 1 Year"
)
@
The partial dependence surface of Figure~\ref{fig:surface3d} shows risk adjusted survival on the Z-axis against values of Bilirubin and Albumin. We again use a linear interpolation between the 2500 estimates, and color the surface by the response. Here blue corresponds to lower and yellow to higher risk adjusted survival. 

We comparing Figure~\ref{fig:surface3d} to the partial coplot for groups of \code{albumin} in Figure~\ref{fig:bili-albumin}, by selecting contour lines parallel to the Bilirubin axis. We can see how lines from higher \code{albimun} levels are close together, and have similar survival response resulting in the over plotting in Figure~\ref{fig:bili-albumin}.

We compare Figure~\ref{fig:surface3d} to the partial coplot for groups of \code{bili} in Figure~\ref{fig:albumin-bili}, by examining contour lines perpendicular to the Bilirubin axis. Here lines are more evenly distributed.
