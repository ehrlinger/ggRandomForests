...
st <- lapply(indx, function(ind){
  lines3D(x=srf$x[ind], 
          ...
          add=TRUE, col="blue", lwd=6)})
@
The partial dependence surface of Figure~\ref{fig:surface3d} shows risk adjusted survival on the Z-axis against values of Bilirubin and Albumin. We again use linear interpolation between the 2500 estimates, and color the surface by the response. Here blue corresponds to lower and yellow to higher risk adjusted survival. The blue lines are placed at the cut points between groups of \code{albumin} and \code{bili} used in the partial coplots of Figures~\ref{fig:bili-albumin} and~\ref{fig:albumin-bili} respectively.
