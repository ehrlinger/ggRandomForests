...
The vignette is written in \LaTeX using the \pkg{knitr} package, which lets us combine \proglang{R} code into documents, weaving results and figures into the dialog text. Throughout this document, \proglang{R} code will be displayed in \emph{code blocks} as shown below. This code block loads all \proglang{R} packages required to compile this vignette, and run the \proglang{R} code listed in the remaining code blocks.
<<libraries, echo=TRUE>>=
#################
# Load_packages #
#################
...
The \code{gg_error} function operates on the random forest object to extract the error estimates from the forest is grown. The following code block first creates a \code{gg_error} object, then uses the \code{plot.gg_error} function to create a \code{ggplot} object for display.
