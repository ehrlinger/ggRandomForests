...
A second engine, `varPro` [@Lu2024varpro; @Ishwaran:varPro:software:2026],
...
`ggRandomForests` works with three upstream fit mappings:
`randomForestSRC::rfsrc()` -> `rfsrc`, `varPro::varpro()` -> `varpro`, and
`randomForestRHF::rhf()` -> `rhf`. RHF support is only an overview here; the
dedicated RHF vignette carries the worked longitudinal analysis.
