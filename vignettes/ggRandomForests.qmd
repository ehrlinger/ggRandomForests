...
`varPro::varpro()` -> `varpro`. It also reads `randomForest` fits. A fitted
forest carries a lot of information, but getting at it usually means digging
through list structures that were never meant to be plotted directly.
**ggRandomForests** does that digging for you, pulling tidy data objects out
of the fit so they drop straight into the `ggplot2` workflows you already
know. The `varPro` engine [@Lu2024varpro; @Ishwaran:varPro:software:2026]
powers a parallel family of functions for release-rule importance and related
diagnostics; that family is covered in the companion vignette referenced at
the end.
