...
author:
  ...
  - name: Eugene H. Blackstone
    affiliation: Cleveland Clinic
title:
  formatted: "\\pkg{ggRandomForests}: Random Forests for Survival"
  # If you use tex in the formatted title, also supply version without
  plain:     "ggRandomForests: Random Forests for Survival"
  # For running headers, if needed
  short:     "Random Forests for Survival"
abstract: >
...
keywords:
  formatted: survival, vimp, \\proglang{R}, \\pkg{randomForestSRC},minimal\ depth, random\ forest
  plain:     survival, vimp, R, randomForestSRC,minimaldepth, randomforest
preamble: >
  \usepackage{amsmath}
  \usepackage{longtable}
  \usepackage{booktabs}
  #bibliography: "vignettes/ggRandomForests.bib"
...
## Random Forests for Survival \\label{S:rfs}
...
# Data Summary: Primary Biliary Cirrhosis (PBC) Data}
...
labels <- c("survival time in days","censoring indicator", "1=D-penicillamine, 2=placebo",
"age in days","0=female, 1=male", "presence of asictes","presence of hepatomegaly",
...
%' 
%' ```{r  dtaViewCat, fig.cap="Categorical Variable measures over time.", fig.width=7.5, fig.height=4}
...
%' ```
%' 
%' ```{r  dtaView, fig.cap="Continuous Variable measures over time.", fig.width=7.5, fig.height=4}
...
```{r  rfsrc, echo=TRUE,eval=FALSE}
pbc_rf <- rfsrc(Surv(days, status) ~ ., data = pbc, 
                  ntree = 2000,
                  na.action="na.impute", 
                  fast.restore = TRUE)
```

```{r  read-forest, echo=FALSE, results=FALSE}
...
```{r  rfsrc-plot, fig.cap="Freedom from pump thrombosis"}
...
## Variable Importance \\label{S:vimp}
Unlike in the linear model settings, Random Forests does not require explicitly specify the functional form of the covariates to the response. Instead, we ascertain which variables contribute to the Random Forest estimates by querying the forest for variable usage. 
...
```{r  rf-vimp, echo=TRUE, fig.cap="Variable Importance"}
plot.gg_vimp(pbc_rf)+
  theme(legend.position="none")
```

## Minimal Depth \\label{S:minimalDepth}
...
```{r  mindepth-view, eval=FALSE, echo=TRUE}
...
```{r  mindepth-plot, fig.cap="Minimal Depth Plot"}
plot.gg_minimal_depth(ggMindepth)
```
...
```{r  rfsrc-plot3Mnth, echo=TRUE,fig.cap="Freedom From Pump Thrombosis"}
ggRFsrc +
  geom_vline(aes(xintercept=364.25), linetype="dashed")+
  coord_cartesian(x=c(0,3*364.25))
```

```{r  variable-plotbili, echo=TRUE, fig.cap="Freedom From Pump Thrombosis at 3 months"}
ggrf <- gg_variable(pbc_rf, time=364.25)
...
```{r  variable-plotAlbumin, echo=TRUE, fig.cap="Survival at 1 year"}
plot(ggrf, x_var = "albumin") +
  theme(legend.position="none")+
...
```{r  variable-plotProthrombin, echo=TRUE, fig.cap="Survival at 1 year"}
plot(ggrf, x_var = "prothrombin") +
  theme(legend.position="none")+
  ...
  scale_shape_manual(values=event.marks, labels=event.labels)
```
```{r  variable-plotCopper, echo=TRUE, fig.cap="Survival at 1 year"}
plot(ggrf, x_var = "copper") +
  theme(legend.position="none")+
  ...
  scale_shape_manual(values=event.marks, labels=event.labels)
```
```{r  variable-plotAge, echo=TRUE, fig.cap="Survival at 1 year"}
plot(ggrf, x_var = "age") +
  theme(legend.position="none")+
  ...
  scale_shape_manual(values=event.marks, labels=event.labels)
```
## Partial Dependence \\label{S:partialDependence}
...
```{r  pbc-partial, echo=TRUE, eval=FALSE}
pDat <- plot.variable(pbc_rf, surv.type="surv", time=c(1,3)*364.25, 
                      xvar.names=xvar, partial=TRUE,
                      show.plots = FALSE)
ggPrtl <- gg_partial(pDat )
```
...
```{r  pbc-partial-bili, echo=TRUE, fig.cap="Risk adjusted freedom From Pump Thrombosis at 3 months"}
plot(ggPrtl[[1]], se=FALSE)+
  theme(legend.position=c(.2,.2))+
...
# Variable Interactions \\label{S:interactions}
