...
* The varPro vignette now documents which variables a `varpro` fit actually
  ...
  ranking -- and `partialpro()` silently drops any requested name outside the
  first set. The new section covers naming `xvar.names` to get past the reported
  ranking, `split.weight = FALSE` to widen the candidate set itself, and the two
  arguments (`nvar`, `sparse`) that look like they should help and don't.
