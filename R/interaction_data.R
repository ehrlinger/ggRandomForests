...
####**********************************************************************
####**********************************************************************
#' Cached \code{\link[randomForestSRC]{find.interaction}} matrix objects for 
#' examples, diagnostics and vignettes.
#'  
#' Data sets storing \code{\link[randomForestSRC]{find.interaction}} matrix 
#' objects corresponding to training data according to the following naming 
#' convention: 
#'\itemize{
...
#'   
#' @details 
#' Constructing the minimal depth interaction matrices on randomForestSRC 
#' objects are computationally expensive. We cache 
#' \code{\link[randomForestSRC]{find.interaction}} matrix objects to improve 
#' the \code{ggRandomForests} examples, diagnostics and vignettes run times. 
...
#' (see \code{\link{rfsrc_data}}), then calculate the minimal depth variable 
#' interaction table with \code{\link[randomForestSRC]{find.interaction}}. Each 
...
#' 
#' \itemize{
#' \item \code{interaction_iris} - The famous (Fisher's or Anderson's) 
#' \code{iris} data set gives the measurements in centimeters of the variables 
#' sepal length and width and petal length and width, respectively, for 50 
#' flowers from each of 3 species of iris. Build a classification random forest 
#' for predicting the species (setosa, versicolor, and virginica) on 5 variables
#' (columns) and 150 observations (rows).
#' 
#' \item \code{interaction_airq} - The \code{airquality} data set is from the 
#' New York State Department of Conservation (ozone data) and the National 
#' Weather Service (meteorological data) collected in New York, from May to 
#' September 1973. Build regression random forest for predicting \code{Ozone} 
#' on 5 covariates and 153 observations.
#' 
#' \item \code{interaction_mtcars} - The \code{mtcars} data was extracted from 
...
#' observations. 
#' 
#' \item \code{interaction_pbc} - The \code{pbc} data from the Mayo Clinic trial
#' in primary biliary cirrhosis (PBC) of the liver conducted between 1974 and 
#' 1984. A total of 424 PBC patients, referred to Mayo Clinic during that 
#' ten-year interval, met eligibility criteria for the randomized placebo 
#' controlled trial of the drug D-penicillamine. 312 cases participated in 
#' the randomized trial and contain largely complete data. Data from the 
#' \code{randomForestSRC} package. Build a survival random forest for 
#' time-to-event death data with 17 covariates and 312 observations (remaining 
#' 106 observations are held out).
#' 
#' \item \code{interaction_veteran} - Veteran's Administration randomized trial
#' of two treatment regimens for lung cancer. Build a survival random forest for
#' time-to-event death data with 6 covariates and 137 observations.
#' }
#' 
