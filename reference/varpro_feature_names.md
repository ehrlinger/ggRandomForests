...
for each level: `sex` becomes `sex0` and `sex1`. To map those back, this
function strips the suffix one character at a time until every name in
`varpro_names` matches a column in `dataset`.
