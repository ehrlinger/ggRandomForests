...
- CRAN rejects a package whose overall `R CMD check` exceeds about 10
  minutes even at 0/0/0. The current baseline is about 3.8 minutes of
  timed steps, dominated by the vignette rebuild (53s) and
  `--run-donttest` examples (35s). Watch that budget when adding either.
